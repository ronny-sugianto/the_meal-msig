  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((callback) {
      setState(() {
        ...
        context.read<FavoriteActionCubit>().getFavoriteById(_data?.id ?? '-');
      ...
      ? BlocProvider(
          ...
          child: Scaffold(
            ...
            appBar: DefaultAppBar(
              title: _data!.name,
              actionWidget: BlocBuilder<FavoriteActionCubit, BaseState>(
                builder: (context, favoriteState) {
                  return IconButton(
                    onPressed: () {
                      if (favoriteState.data == true) {
                        return context
                            .read<FavoriteActionCubit>()
                            .removeFavoriteById(_data?.id ?? '-');
                      }

                      context
                          .read<FavoriteActionCubit>()
                          .setFavoriteById(_data!);
                    },
                    icon: Icon(
                      favoriteState.data == true
                          ? Icons.favorite
                          : Icons.favorite_border,
                      color: favoriteState.data == true ? Colors.red : null,
                    ),
                  );
                },
              ),
